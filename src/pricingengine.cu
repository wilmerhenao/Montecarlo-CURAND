...
__device__ inline float calculateStep(float &drift, float &diffusion, curandState &state){
    return expf(drift + diffusion * curand_normal( &state ));
}
__device__ inline double calculateStep(double &drift, double &diffusion, curandState &state){
    return exp(drift + diffusion * curand_normal_double(&state));
}

__device__ inline float getRandom(curandState & state, float stuff){
  return curand_normal(&state) * stuff;
}

__device__ inline double getRandom(curandState & state, double stuff){
  return curand_normal_double(&state) * stuff;
}

// 
template <typename Real>
__global__ void sampleCuRand(Real * d_paths, 
  ...
  for (unsigned int i = tid; i < numSims; i += step){
    pos = i;    
    for ( unsigned int t = 0; t < numTimeSteps; t++, pos += numSims){
      d_paths[tid] = getRandom(localState, static_cast<Real> (1));
    }
  }
}
...
__global__ void generatePaths(Real * const paths,
    ...
    for (unsigned int i = tid ; i < numSims ; i += step){
        ...
        for (unsigned int t = 0 ; t < numTimesteps ; t++, output += numSims){
            s *= calculateStep(drift, diffusion, localState);
            *output = s;
        }
    }
}
...
// Asian Lookback Knock-Out Valuation kernel
template <typename Real>
__global__ void computeValueALK(Real * const values,
	...
	Real finval = static_cast<Real>(1);
	Real valmin = static_cast<Real>(1);
	Real killer = static_cast<Real>(1);
        for (unsigned int t = 0 ; t < numTimesteps ; t++, path += numSims){
	  finval = *path;
	  avg += finval;
	  if (valmin > finval){
	    valmin = finval;
	  }else if(finval * option->spot > option->barrier){
	    killer = static_cast<Real>(0);
	  }
        }
        avg = avg * option->spot / numTimesteps;
        // Compute the payoff
        Real payoff = avg - valmin * option->spot;
        if (option->type == genericOption<Real>::Put){
            payoff = - payoff;
        }
        payoff = killer * max(static_cast<Real>(0), payoff);
...
template <typename Real>
__global__ void computeValueBarrier(Real * const valuesko,
				       Real * const valueski,
				       const Real * const paths,
				       const genericOption<Real> * const option,
				       const unsigned int numSims,
	...
	killer = static_cast<Real>(1);
        ...
        finvalknockout = finval * option->spot;
	finvalknockin = finval * option->spot;
        // Compute the payoff
	...
	Real payoffknockin = finvalknockin - option->strike;
        ...
        payoffknockout = killer * max(static_cast<Real>(0), payoffknockout);
        payoffknockin = (static_cast<Real>(1) - killer) * max(static_cast<Real>(0), payoffknockin);
        // Accumulate payoff locally
...
void successMessage(cudaError_t cudaResult){
...
void noAttribs(cudaError_t cudaResult){
  if (cudaResult != cudaSuccess){
...
void PricingEngine<Real>::operator()(genericOption<Real> &option){
    ...
    if (cudaResult != cudaSuccess){
        std::string msg("Could not set CUDA device: ");
        msg += cudaGetErrorString(cudaResult);
        throw std::runtime_error(msg);
    }
    ...
    // Get computeValueBarrier function properties and check the maximum block size
    cudaResult = cudaFuncGetAttributes(&funcAttributes, computeValueBarrier<Real>);
    noAttribs(cudaResult);
    if (block.x > (unsigned int)funcAttributes.maxThreadsPerBlock){
        throw std::runtime_error("Block X dimension is too large for computeValueBarrier kernel");
    }
    ...
    if (cudaResult != cudaSuccess){
        std::string msg("Could not allocate memory on device for option data: ");
        msg += cudaGetErrorString(cudaResult);
        throw std::runtime_error(msg);
    }
    ...
    if (cudaResult != cudaSuccess) {
        std::string msg("Could not copy data to device: ");
        msg += cudaGetErrorString(cudaResult);
        throw std::runtime_error(msg);
    }
    ...
    if (cudaResult != cudaSuccess){
        std::string msg("Could not allocate memory on device for paths: ");
        msg += cudaGetErrorString(cudaResult);
        throw std::runtime_error(msg);
    }
    ...
    if (cudaResult != cudaSuccess){
        std::string msg("Could not allocate memory on device for RNG state: ");
        msg += cudaGetErrorString(cudaResult);
        throw std::runtime_error(msg);
    }
    ...
    if (cudaResult != cudaSuccess){
        std::string msg("Could not allocate memory on device for partial results: ");
        msg += cudaGetErrorString(cudaResult);
        throw std::runtime_error(msg);
    }
    ...
    double elapsedTime2;
    shrDeltaT(0);
    printf("Size of grid is %i and size of block is %i", grid.x, block.x);
    // Initialise RNG
...
///////////////////////////////////////////////////////////////////
    ...
    successMessage(cudaResult);
    // Complete sum-reduction on host
    option.valuePlainVanilla = std::accumulate(values.begin(), values.end(), static_cast<Real>(0));
    // Discount to present value
    option.valuePlainVanilla *= exp(- option.r * option.tenor) / m_numSims; ;
    elapsedTime2 = shrDeltaT(0);
    printf("\n Time Spent working on Plain Vanilla option in the GPU is: %f\n\n", elapsedTime2);
/////////////////////////////////////////////////////////////////////
    // Compute values Asian Lookback Knock-Out Option
    computeValueALK<<<grid, block, block.x * sizeof(Real)>>>(d_values, d_paths, d_option, m_numSims, numTimesteps);
    // Copy partial results back
    cudaResult = cudaMemcpy(&values[0], d_values, grid.x * sizeof(Real), cudaMemcpyDeviceToHost);
    successMessage(cudaResult);
    // Complete sum-reduction on host
    option.valueALK = std::accumulate(values.begin(), values.end(), static_cast<Real>(0));
    // Discount to present value
    option.valueALK *= exp(- option.r * option.tenor)/ m_numSims;
...
/////////////////////////////////////////////////////////////////////
    ...
    // Discount to present value
    option.valueAsian *= exp(- option.r * option.tenor) / m_numSims;
///////////////////////////////////////////////////////////////////
    ...
    successMessage(cudaResult);
    computeValueBarrier<<<grid, block, block.x * sizeof(Real)>>>(d_values, d_values2, d_paths, d_option, m_numSims, numTimesteps);
    // Copy partial results back
    cudaResult = cudaMemcpy(&values[0], d_values, grid.x * sizeof(Real), cudaMemcpyDeviceToHost);
    successMessage(cudaResult);
    // Complete sum-reduction on host
    ...
    // Discount to present value
    option.valueKnockout *= exp(- option.r * option.tenor) / m_numSims;
    ...
    // Discount to present value
    option.valueKnockin *= exp(- option.r * option.tenor) / m_numSims;
///////////////////////////////////////////////////////////////////
    ...
    // Discount to present value
    option.valueLookback *= exp(- option.r * option.tenor)/ m_numSims;

    ///////////////////////////////////////////////////////////////////

    std::vector<Real> paths(m_numSims * numTimesteps);

    std::ofstream showoutput;
    showoutput.open("samplerandomnumsAcross.txt");
    // Generate a few random numbers for study
    sampleCuRand<Real><<<grid, block>>>(d_paths, d_rngStates, m_numSims, numTimesteps);
    cudaResult = cudaMemcpy(&paths[0], d_paths, m_numSims * numTimesteps, cudaMemcpyDeviceToHost);

    // Write down the first number generated by each thread    
    for (unsigned int i = 0; i < (m_numSims); i++ ){
      //Real thisval = d_paths.at(i);
      showoutput << paths[i] << std::endl;
    }
    showoutput.close();

    showoutput.open("samplerandomnumsAlong.txt");
    // Write down the numbers generated in the same thread
 
    for (unsigned int i = 0; i < (m_numSims* numTimesteps); i += numTimesteps ){
      //Real thisval = d_paths.at(i);
      showoutput << paths[i] << std::endl;
    }
    showoutput.close();
