...
// SDK information
static const char *shrLogFile = "MonteCarloSinglegenericOptionP.txt";
...
int main(int argc, char **argv){
    ...
    shrLog("Monte Carlo Path - Dependent Options with cuRand\n");
    shrLog("===========================================\n\n");
...
// here if the device is capable of running double precision I assign double
// if the device can only use float, that's what I use.
	  int deviceIndex;
...
void runTest(int argc, const char **argv){
    try{
        ...
        if (cudaResult != cudaSuccess){
            ...
            throw std::runtime_error("cudaGetDeviceCount");
        }
...

        {
            ...
            if (shrGetCmdLineArgumentstr(argc, argv, "sims", &value)){
                ...
                if (test.numSims < k_sims_min || test.numSims > k_sims_max){
                    ...
                    throw std::invalid_argument("sims");
                }
            }
            else{
            ...
            if (shrGetCmdLineArgumentstr(argc, argv, "block-size", &value)){
                ...
                if (test.threadBlockSize < k_bsize_min || test.threadBlockSize > static_cast<unsigned int>(deviceProperties.maxThreadsPerBlock)){
                    shrLogEx(LOGBOTH | ERRORMSG, 0, "specified block size (%d) is not valid, must be between %d and %d for device %d.\n", test.threadBlockSize, k_bsize_min, deviceProperties.maxThreadsPerBlock, test.device);
                    throw std::invalid_argument("block-size");
                }
                if (test.threadBlockSize & (test.threadBlockSize-1)){
                    shrLogEx(LOGBOTH | ERRORMSG, 0, "specified block size (%d) is not valid, must be a power of two (see reduction function).\n", test.threadBlockSize);
                    throw std::invalid_argument("block-size");
                }
            }
            else{
